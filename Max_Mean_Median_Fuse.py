...
class SetNet(nn.Module):
    def __init__(self, hidden_dim):
        ...
        # ---------- joint-fuse 1×1 convs cho 3 cấp độ kênh 32 / 64 / 128 ----------
        self.fuse32  = BasicConv2d(_set_channels[0] * 3, _set_channels[0], 1)
        self.fuse64  = BasicConv2d(_set_channels[1] * 3, _set_channels[1], 1)
        self.fuse128 = BasicConv2d(_set_channels[2] * 3, _set_channels[2], 1)
    ...
    def frame_mean(self, x):
        if self.batch_frame is None:
            return torch.mean(x, 1), None
        else:
            mean_chunks = [
                torch.mean(
                    x[:, self.batch_frame[i]:self.batch_frame[i + 1], :, :, :], 1
                )
                for i in range(len(self.batch_frame) - 1)
            ]
            mean_list = torch.cat(mean_chunks, 0)
            return mean_list, None
    def _stat_fuse(self, x, stage='32'):
      f_max,   _ = self.frame_max(x)
      f_mean,  _ = self.frame_mean(x)
      f_median,_ = self.frame_median(x)

      cat = torch.cat([f_max, f_mean, f_median], dim=1)   # (N, 3C, H, W)

      if stage == '32':
          return self.fuse32(cat)
      elif stage == '64':
          return self.fuse64(cat)
      else:  # '128'
          return self.fuse128(cat)
    ...
    def forward(self, silho, batch_frame=None):
        ...
        x = self.set_layer1(x)
        x = self.set_layer2(x)
        gl = self.gl_layer1(self._stat_fuse(x,'32'))
        gl = self.gl_layer2(gl)
        gl = self.gl_pooling(gl)

        x = self.set_layer3(x)
        x = self.set_layer4(x)
        gl = self.gl_layer3(gl + self._stat_fuse(x,'64'))
        gl = self.gl_layer4(gl)

        x = self.set_layer5(x)
        x = self.set_layer6(x)
        x = self._stat_fuse(x,'128')
        gl = gl + x
